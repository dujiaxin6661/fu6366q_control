 ...
 * - GP33 + GP45 together immediately commands 0 rpm for brake-stop tests.
 * - Target speed is clamped to -200..+200 rpm.
 * - After boot, the motor enters position hold and moves to POSITION_HOLD_TARGET_DEG.
 */
...
static void Apply_ConfigPositionHold(void)
{
    mcFocCtrl.CtrlMode = PostionLoopMode;
    mcFocCtrl.CurrentLoopEn = 1;
    mcFocCtrl.AngleMode = AngleLoop;
    mcFocCtrl.TargetAngle.s32 = POSITION_DEG_TO_RAW(POSITION_HOLD_TARGET_DEG);
