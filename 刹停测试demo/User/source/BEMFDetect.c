...
{
    uint8 BEMFStatus = 0;
    if(ReadBit(CMP_SR , CMP2OUT)) BEMFStatus += 4;
    if(ReadBit(CMP_SR , CMP1OUT)) BEMFStatus += 2;
    if(ReadBit(CMP_SR , CMP0OUT)) BEMFStatus += 1;
    return BEMFStatus;
}
...
uint8 CWCCWDetect(uint8 HallStatus)
{
    ...
    if(MC_HallStatus != HallStatus)
    {
        ...
        {
            case 1:
                if(HallStatus == 5) MC_FR = CCW;
                if(HallStatus == 3) MC_FR = CW;
            break;

            case 2:
                if(HallStatus == 3) MC_FR = CCW;
                if(HallStatus == 6) MC_FR = CW;
            break;

            case 3:
                if(HallStatus == 1) MC_FR = CCW;
                if(HallStatus == 2) MC_FR = CW;
            break;

            case 4:
                if(HallStatus == 6) MC_FR = CCW;
                if(HallStatus == 5) MC_FR = CW;
            break;

            case 5:
                if(HallStatus == 4) MC_FR = CCW;
                if(HallStatus == 1) MC_FR = CW;
            break;
            case 6:
                if(HallStatus == 2) MC_FR = CCW;
                if(HallStatus == 4) MC_FR = CW;
            break;
        }
        MC_HallStatus = HallStatus;
    }
...
//        BEMFDetect.BemfAngle  = ThetaDispose (BEMFDetect.BEMFStatus,mcHall.GivenFRStatus,TempAngleOffsetCW,TempAngleOffsetCCW);
        //根据BEMF状态判断FR状态
        ...
        BEMFDetect.BEMF_Status[BEMFDetect.BEMFStep] = BEMFDetect.BEMFStatus;
        //速度计算
        BEMFSpeedCal();
...
//        {
//          //超过设定转速时直接启动
//          if((mcHall.HallSpeed > BEMFMotorStartSpeed) && (mcHall.HallSpeed < BEMFMotorStartSpeedHigh))
//          {
//            if ((mcHall.HallStatus_Result == 2))
//            {
//                //执行直接闭环启动程序
//                BEMFFOCCloseLoopStart();
//                ClrBit(CMP_CR0 , CMP2IM1 | CMP2IM0 | CMP1IM1 | CMP1IM0 | CMP0IM1 | CMP0IM0);
...
//          }
//        }
//        else if(mcHall.HallFRStatus == 2)//反转，刹车
//        {
...
//            if(mcHall.HallSpeed < BEMFMotorStartSpeed)
//            {
//               ClrBit(CMP_CR0 , CMP2IM1 | CMP2IM0 | CMP1IM1 | CMP1IM0 | CMP0IM1 | CMP0IM0);
//               ClrBit(CMP_CR2 , CMP0EN);
//               mcState                           = mcPosiCheck;
...
//    FOC__EOME = mcHall.HallSpeed;
//    mcSpeedRamp.ActualValue =mcHall.HallSpeed;
    #if (EstimateAlgorithm == SMO)
    {
        //根据不同转速确启动的ATO_BW值
        if (BEMFDetect.BEMFSpeed > _Q15(1200.0 / MOTOR_SPEED_BASE))
        {
        ...
        else if (BEMFDetect.BEMFSpeed > _Q15(800.0 / MOTOR_SPEED_BASE))
        {
