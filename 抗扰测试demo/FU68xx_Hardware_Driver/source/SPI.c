...
//  
////  SetReg(SPI_CR1 , NSSMOD0 | NSSMOD1 , 0); //双线DBG模式
//       

////		ClrBit(SPI_CR1,NSSMOD1);//01 单线模式
...
//    SPIIE = 0;                                      // SPI中断使能
//    SPI_CLK = 1;		                            // Fspi = Fcpu / (2*(SPI_CLK + 1)) = 6MHz
...
void SPI_Init(void)
{
    ...
    /*  -------------------------------------------------------------------------------------------------
        ...
        -------------------------------------------------------------------------------------------------*/
    SetReg(SPI_CR0, CPHA | CPOL, CPOL);
//	      ClrBit(PH_SEL1 , SPICT0);// 0:P05 1:P00
//        ClrBit(PH_SEL2 , SPICT1);// 0:P05 1:P34  20210907 add
...
//        ClrBit(SPI_CR1,NSSMOD1);// 双线模式，MCU的 SCLK MOSI     接SPI小板的 SCLK  MOSI，       SPI拨码开关：SW2 = 1;SW2 = 0;  
//        ClrBit(SPI_CR1,NSSMOD0); // 双线模式，MCU的 SCLK MOSI     接SPI小板的 SCLK  MOSI，       SPI拨码开关：SW2 = 1;SW2 = 0;  



//        SetBit(SPI_CR1,NSSMOD1);     // 三线模式，MCU的 SCLK NSS MOSI 接SPI小板的 SCLK NSS MOSI，    SPI拨码开关：SW2 = 0;SW3 = 0;    
//        SetBit(SPI_CR1,NSSMOD0);     // 三线模式，MCU的 SCLK NSS MOSI 接SPI小板的 SCLK NSS MOSI，    SPI拨码开关：SW2 = 0;SW3 = 0; 
				
				ClrBit(SPI_CR1,NSSMOD1);   //单线模式MCU的 SCLK          接SPI小板的 MOSI，             SPI拨码开关：SW2 = 1;SW3 = 1;   
        SetBit(SPI_CR1,NSSMOD0);   //单线模式MCU的 SCLK          接SPI小板的 MOSI，             SPI拨码开关：SW2 = 1;SW3 = 1;   
